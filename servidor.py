...
class Server:
    ...
    def handle_client(self, client_socket):
        ...
        while True:
            ...
            elif msg_client.startswith("CADASTRAR"):
    ...
    def cadastrar_item_cardapio(self, client_socket, msg_client):
        with self.__lock_pedidos:
            _, nome_cadastro, descricao  = msg_client.split("/")

            item_cardapio = cardapio.get(nome_cadastro)
            if item_cardapio is None:
                prato = Prato(nome_cadastro, descricao, preco)
                cardapio.put(prato.nome_cadastro, prato.descricao)
    ...
    def realizar_pedido(self, client_socket, celular_registrado, msg_client):
        with self.__lock_pedidos:
            ...
            _, nome_produto = msg_client.split()

            produto = cardapio.get(nome_produto)

            if produto is not None:
                pedidos_cliente = self.__clientes.buscar(num)
                pedidos_cliente.append(nome_produto)
                self.__clientes.set_valor(num, pedidos_cliente)
                resposta = "202"  # Pedido realizado com sucesso
            else:
                resposta = "401"  # Produto não encontrado 
